...
describe("src/main.js", () => {
  ...
  describe("create()", () => {
    ...
    it("should resolve with a promise containing the newly saved student", async () => {
      ...
      const expected =  await create(student);
      expect(response).toEqual(expected);
    });
   ...
   describe("show()", () => {
    ...
    it("should resolve with a promise containing the student data", async () => {
      axios.post.mockImplementation(() => Promise.resolve({ id }));
      const response = await show();
      const expected = await show(id);
      expect(response).toEqual(expected);
    });
     
    it("should log an error to the console", async () => {
      axios.post.mockImplementation(() =>
      Promise.reject(newError("GET request failed."))
      );
      jest.spyOn(console, "error");
